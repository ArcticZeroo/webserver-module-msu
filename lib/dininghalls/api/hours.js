...
const CLOSE_TIME_REGEX = /(.+) close(?:s)? at\s*(.+)?/i;
...
function parseSingleData(rawData, meal) {
    ...
    if (extra) {
        ...
        for (const extraPiece of extraPieces) {
            ...
            } else if (CLOSE_TIME_REGEX.test(extra)) {
                const grillCloseMatch = extra.match(CLOSE_TIME_REGEX);
        ...
        if (LIMITED_MENU_REGEX.test(extra)) {
            ...
            const [, venue, rawTime] = extra.match(CLOSE_TIME_REGEX);
        ...
        } else if (OPEN_TIME_REGEX.test(extra)) {
            const [, venue, rawTime] = extra.match(OPEN_TIME_REGEX);
