...
const { findBySearchName } = require('../hall-storage');
const { MealIdentifier } = require('../api');
...
function serializeToKey({ diningHall, menuDate, meal }) {
    return [diningHall.searchName, menuDate.getFormatted(), meal].join('|');
}

async function deserializeFromKey(key) {
    const [searchName, date, meal] = key.split('|');

    let diningHall;
    try {
        diningHall = await findBySearchName(searchName);
    } catch (e) {
        throw e;
    }
...
function getRequestUrl({ diningHall, date, meal }) {
...
async function getDiningHallMenu({ diningHall, menuDate, meal }) {
    const url = getRequestUrl({ diningHall, date: menuDate.getFormatted(), meal });
...
const cache = new ExpiringCache(
    async key => getDiningHallMenu(await deserializeFromKey(key)),
    12 * 60 * 60 * 1000
);

function retrieveMenu(diningHall, menuDate, meal) {
    return cache.getEntry(serializeToKey({ diningHall, menuDate, meal }));
}

module.exports = { retrieveMenu, MenuDate };