...
const FoodTruckMenuModule = require('./menu');

const htmlCacheKey = 'foodTruck_html';
const stopsCacheKey = 'foodTruck_stops';
...
async function retrieveStopsFromWeb() {
    let page;
    try {
        page = await cache.getValue(htmlCacheKey);
    } catch (e) {
        throw e;
    }
...
cache.add(htmlCacheKey, { expireTime: 15 * 60 * 1000, fetch: retrieveFoodTruckHtml });
cache.add(stopsCacheKey, { expireTime: 15 * 60 * 1000, fetch: retrieveStopsFromWeb });
    ...
    start() {
        ...
        router.get('/list', function (req, res) {
            async function run() {
                let stops;
                try {
                    stops = await cache.getValue(stopsCacheKey);
                } catch (e) {
                    throw e;
                }
