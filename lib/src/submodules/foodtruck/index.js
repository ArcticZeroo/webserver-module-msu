...
async function retrieveFoodTruckHtml() {
    return retryingRequest_1.default(config.pages.FOOD_TRUCK);
}
async function retrieveStopsFromWeb() {
    let page;
    try {
        page = await index_1.cache.getValue(index_1.CacheKey.foodTruckHtml);
    }
    catch (e) {
        throw e;
    }
...
index_1.cache.add(index_1.CacheKey.foodTruckHtml, { fetch: retrieveFoodTruckHtml });
class FoodTruckModule extends webserver_module_1.default {
    ...
    start() {
        ...
        router.get('/list', index_1.handleEndpoint(index_1.CacheKey.foodTruckStops, retrieveStopsFromWeb));
        this.app.use('/api/msu/foodtruck/', router);
        this.loadChild(index_2.default);
    }
}
FoodTruckModule.IDENTIFIER = 'Food Truck';
