...
class UabModule extends webserver_module_1.default {
    ...
    static async retrieveSingleCalendarEventFromWeb(url) {
        let htmlResponse;
        try {
            htmlResponse = await retryingRequest_1.default(config.pages.UAB_EVENT_BASE + url);
        }
        catch (e) {
            throw e;
        }
    ...
    static async retrieveCalendarEventListFromWeb() {
        let htmlResponse;
        try {
            htmlResponse = await retryingRequest_1.default(config.pages.UAB_CALENDAR);
        }
        catch (e) {
            throw e;
        }
    ...
    start() {
        ...
        router.get('/list', index_1.handleEndpoint(index_1.CacheKey.uabEvents, UabModule.retrieveCalendarEventListFromWeb));
