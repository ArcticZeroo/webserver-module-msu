...
class FoodTruckMenuModule extends webserver_module_1.default {
    ...
    static async retrieveMenusFromWeb() {
        let htmlResponse;
        try {
            htmlResponse = await index_1.cache.getValue(index_1.CacheKey.foodTruckHtml);
        }
        catch (e) {
            throw e;
        }
    ...
    start() {
        ...
        router.get('/api/msu/foodtruck/menu', index_1.handleEndpoint(index_1.CacheKey.foodTruckMenu, FoodTruckMenuModule.retrieveMenusFromWeb));
        this.app.use(router);
    }
}
