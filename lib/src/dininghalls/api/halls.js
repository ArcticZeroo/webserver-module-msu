...
const NAME_FROM_URL = new RegExp(`^${config_1.default.pages.DINING_HALL_MENU}(.+?)/all`);
/**
...
async function retrieveDiningHalls() {
    let body;
    try {
        body = await retryingRequest_1.default(config_1.default.pages.EAT_AT_STATE);
    }
    catch (e) {
        throw e;
    }
    ...
    nameSquares.each(function () {
        ...
        if (!url.toLowerCase().startsWith(config_1.default.pages.DINING_HALL_MENU)) {
            return;
        }
